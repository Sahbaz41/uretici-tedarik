...
export const ProductsSection: React.FC<ProductsSectionProps> = ({
  ...
  const getBadgeStyle = (badgeType: string) => {
    switch (badgeType) {
      case 'primary':
        return 'bg-[#2563eb] text-[#eeefff] border border-[#2563eb]/60 shadow-[0_0_12px_rgba(37,99,235,0.4)]';
      case 'tertiary':
        return 'bg-[#00788c] text-[#d7f6ff] border border-[#4cd7f6]/40 shadow-[0_0_12px_rgba(0,120,140,0.3)]';
      case 'secondary':
        return 'bg-[#d97707] text-[#432100] border border-[#ffb77d]/40 shadow-[0_0_12px_rgba(217,119,7,0.3)]';
      case 'surface-tint':
        return 'bg-[#1e1f25] text-[#b4c5ff] border border-[#b4c5ff]/50';
      case 'error':
  ...
  const getCurrencyDisplay = (priceStr: string) => {
    ...
    // Matches numbers like $65,00 or $2.000,00 or $1,60
  ...
  return (
    <section id="urunler-bolumu" className="w-full py-16 lg:py-24 relative overflow-hidden">
      {/* İnce arka plan degradeleri */}
      <div className="absolute top-1/3 left-10 w-96 h-96 bg-[#2563eb]/5 blur-[120px] pointer-events-none rounded-full" />
      <div className="absolute bottom-10 right-10 w-96 h-96 bg-[#00788c]/5 blur-[120px] pointer-events-none rounded-full" />
      ...
      <div className="w-full px-4 sm:px-6 mx-auto max-w-[90rem] flex flex-col gap-10 relative">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b border-[#434655]/30">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <span className="w-2 h-2 rounded-full bg-[#ffb77d] animate-pulse" />
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            {selectedCategory && (
              <button
                ...
                className="px-3.5 py-1.5 rounded-xl bg-[#2563eb]/20 text-[#b4c5ff] hover:bg-[#2563eb]/30 text-xs font-semibold border border-[#2563eb]/50 transition-all flex items-center gap-1.5 cursor-pointer"
              >
                <RotateCcw className="w-3.5 h-3.5" />
                <span>Filtreyi Temizle (Tümü: {PRODUCTS.length})</span>
              </button>
            )}
            <span className="px-3 py-1.5 rounded-xl bg-[#1a1b21] border border-[#434655]/40 font-mono text-xs text-[#8d90a0]">
              Kur: <strong className="text-[#e2e2e9]">{currency}</strong> (+KDV Toptan)
            </span>
            <span className="px-3 py-1.5 rounded-xl bg-[#282a2f] border border-[#434655]/40 text-[#4cd7f6] text-xs font-mono font-semibold">
              Gebze Çayırova Fabrika Stok
            </span>
          </div>
        </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="p-12 rounded-3xl bg-[#14161d] border border-[#434655]/40 text-center flex flex-col items-center justify-center gap-4 my-8 shadow-2xl">
            <div className="w-16 h-16 rounded-2xl bg-[#282a2f] text-[#ffb77d] flex items-center justify-center border border-[#ffb77d]/30 shadow-lg">
            ...
            <div className="flex items-center gap-3 mt-2">
              <button
                onClick={onClearFilter}
                className="px-4 py-2 rounded-xl bg-[#282a2f] hover:bg-[#33353a] text-xs font-semibold text-[#e2e2e9] transition-colors cursor-pointer"
              >
                Tüm Ürünleri Göster
              </button>
              <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map((prod) => (
              <div
                ...
                className="flex flex-col rounded-2xl bg-[#14161d] border border-[#434655]/30 overflow-hidden hover:border-[#2563eb]/70 hover:shadow-[0_8px_32px_rgba(37,99,235,0.2)] hover:-translate-y-1 transition-all duration-300 group relative"
              >
                {/* Üst Görsel Alanı */}
                <div className="relative h-48 w-full bg-[#0c0e13] overflow-hidden">
                  {prod.imageUrl ? (
                    <img
                      src={prod.imageUrl}
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-emerald-950/80 via-[#14161d] to-[#1e1f25] flex flex-col items-center justify-center p-4">
                      <Memory className="w-16 h-16 text-[#4cd7f6] opacity-50 group-hover:scale-110 transition-transform duration-300" />
                      <span className="font-mono text-[11px] text-[#4cd7f6] mt-2 font-bold">FR-4 EPOXY LAMINATE</span>
                    </div>
                  ) : prod.id === 'peek-termoplastik' ? (
                    <div className="w-full h-full bg-gradient-to-br from-rose-950/80 via-[#14161d] to-[#1e1f25] flex flex-col items-center justify-center p-4">
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-amber-950/70 via-[#14161d] to-[#1e1f25] flex flex-col items-center justify-center p-4">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-[#14161d] via-transparent to-black/30 pointer-events-none" />
                  ...
                  <span className="absolute top-3 right-3 px-2 py-0.5 rounded-lg bg-[#0c0e13]/85 text-[#e2e2e9] font-mono text-[10.5px] backdrop-blur-md border border-[#434655]/40 font-semibold">
                    {prod.secondaryBadge}
                  </span>

                  {/* DIN Normu Çipi (Görselin Alt Solunda) */}
                  {prod.dinNorm && (
                    <span className="absolute bottom-2.5 left-3 px-2 py-0.5 rounded bg-[#0c0e13]/90 text-[#4cd7f6] font-mono text-[10px] border border-[#4cd7f6]/30 backdrop-blur-md font-semibold">
                ...
                <div className="p-4 sm:p-5 flex flex-col flex-1 gap-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-[10px] px-1.5 py-0.5 rounded bg-[#1e1f25] text-[#8d90a0]">
                      {prod.code}
                    </span>
                  </div>
                  ...
                  {/* Mikro Teknik Parametreler Matrisi (AEO & Pro Endüstriyel Çipler) */}
                  <div className="grid grid-cols-2 gap-1.5 p-2.5 rounded-xl bg-[#0c0e13]/70 border border-[#434655]/25 font-mono text-[11px] mt-auto">
                    ...
                    <div className="flex items-center gap-1.5 text-[#c3c6d7] truncate">
                      <Scale className="w-3 h-3 text-[#4cd7f6] shrink-0" />
                  ...
                  <div className="pt-3 border-t border-[#434655]/30 flex items-center justify-between mt-1">
                    <div>
                      ...
                      <span className="font-display text-[15px] sm:text-[16px] text-[#b4c5ff] font-extrabold">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        onClick={() => onOpenTds(prod.id)}
                        className="h-9 w-9 rounded-xl bg-[#1e1f25] hover:bg-[#282a2f] text-[#c3c6d7] hover:text-[#4cd7f6] flex items-center justify-center transition-colors border border-[#434655]/40 cursor-pointer"
                      ...
                      <button
                        ...
                        className="h-9 px-3.5 rounded-xl bg-[#2563eb] hover:bg-[#0053db] text-white text-xs font-semibold flex items-center gap-1.5 shadow-[0_0_16px_rgba(37,99,235,0.3)] transition-all cursor-pointer group-hover:shadow-[0_0_20px_rgba(37,99,235,0.5)]"
                      >
